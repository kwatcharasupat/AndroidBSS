...
public class AuxIVAParallelActivity_DevMode extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.devmodeactivity_auxiva_parallel);
        ...
        istftButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    void runAuxIVA() {
        ...
        //startMethodTracingSampling("AuxIVAParallelThread_VDemix", 2147483647, 1);
        ...
        auxIVA = new AuxIVAParallel(STFTdata, nItr, true, null, "norm", cFuncParam);
    ...
    private class ISTFTrunnable implements Runnable {
        ...
        public void run() {
            ...
            /*
            for (int c = 0; c < nChannels; c++) {
                Log.i("DEBUG", "Reconstructed sig: = " + Arrays.toString(reSig[c]));
            }
            */

            IntStream.range(0, nChannels).parallel().forEach(c -> {
                double max = Arrays.stream(reSig[c]).max().getAsDouble();
    ...
    private class WriteToFileRunnable implements Runnable {
        ...
        public WriteToFileRunnable(int channel) {
        ...
        public void run() {
            ...
            File file = new File(getExternalStorageDirectory().getAbsolutePath(), MainActivity_DevMode.fileNameNoExt + "AuxIVAParallel_Channel" + (chanIdx + 1) + ".wav");
            try {
